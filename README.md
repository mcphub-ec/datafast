...
Servidor Model Context Protocol (MCP) para la integración con **la red Datafast**.
...
-   Procesamiento tradicional de tarjetas en Ecuador.
...
-   `checkout_id`: Genera un ID de checkout para iniciar el pago.
...
```env
DATAFAST_ENTITY_ID="tu_entity_id"
...
docker run -d \
  --name mcp-datafast \
  --env-file .env \
  ghcr.io/mcphub-ec/mcp-datafast:latest
```
...
services:
  mcp-datafast:
    image: ghcr.io/mcphub-ec/mcp-datafast:latest
    container_name: mcp-datafast
    env_file:
      - .env
